...
// --- LOBBIES ---
function createLobby() { return { players: [], state: 'waiting', seed: 12345, matchStats: [], startTime: 0, timer: null }; }
let lobbies = { 'ffa': createLobby(), 'madness': createLobby() };
...
io.on('connection', (socket) => {
    ...
    socket.on('submit_apm', (val) => {
        ...
        if(val > (accounts[socket.username].bestAPM||0)) { accounts[socket.username].bestAPM = parseInt(val); saveAccounts(); }
    });
...
function endGame(type) {
    ...
    // Find winner (force first if won)
